...
class PluginIPCompositionAdapterXL(PluginBase):
    ...
    def setup_pipe(self):
        ...
        self.ctx.pipe = IPAdapterPlusXL(
            self.ctx.pipe,
            image_encoder_path,
            hf_download("ostris/ip-composition-adapter/ip_plus_composition_sdxl.safetensors"),
            self.ctx.device,
            num_tokens=16
        )
